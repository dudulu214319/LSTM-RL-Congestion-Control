...
class Estimator(object):
    def __init__(self, model_path="./model/pretrained_model.pth", step_time=200):
        ...
        # print(os.getcwd())
        self.ppo.policy.load_state_dict(torch.load(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ppo_2021_07_25_04_57_11_with500trace.pth')))
    ...
    def get_estimated_bandwidth(self, packets_dict_list)->int:
        '''
        Calculate estimated bandwidth
        '''
        for pkt_stats in packets_dict_list:
            self.report_states(stats=pkt_stats) # 手动做了隐式的参数输入
        ...
        self.state = torch.roll(self.state, -1, dims=-1) #在时间维度上原地向前滚动
        self.state[0, 0, -1] = self.receiving_rate / 6000000.0
        self.state[0, 1, -1] = self.delay / 1000.0
        self.state[0, 2, -1] = self.loss_ratio
        self.state[0, 3, -1] = self.bandwidth_prediction / 6000000.0 #上一次的预测值
    ...
    # def GCC_estimated_bandwidth(self, packets_dict_list) -> int:
    #     '''
    #     Calculate estimated bandwidth
    #     '''
    #     for pkt_stats in packets_dict_list:
    #         self.gcc_estimator.report_states(stats=pkt_stats) # 手动做了隐式的参数输入

    #     self.gcc_decision, self.overuse_flag = self.gcc_estimator.get_estimated_bandwidth()  # 参数输入呢?
    #     self.bandwidth_prediction = self.gcc_decision
    #     # print("self.bandwidth_prediction:", self.bandwidth_prediction)
