...
from deep_rl.LRCC_ppo_agent import PPO
        ...
        while time_step < update_interval:
            ...
            while not done and time_step < update_interval:
                ...
                if len(throughput_seq) > seq_len: 
                    ...
                    # 将预测值补充到state中
                    _state = torch.cat([torch.Tensor(_state), pred], dim=0)
                else:
                    _state = torch.cat([torch.Tensor(_state), torch.Tensor([_state[0]])], dim=0)
                    
                state = torch.roll(state, -1, dims=-1) #在时间维度上原地向前滚动
                state[0, 0, -1] = _state[0]
                state[0, 1, -1] = _state[1]
                state[0, 2, -1] = _state[2]
                state[0, 3, -1] = _state[3]
                state[0, 4, -1] = _state[4]
