...
from .actor_critic import LRCC_ActorCritic
...
class PPO:
    def __init__(self, state_dim, action_dim, exploration_param, lr, betas, gamma, ppo_epoch, ppo_clip, use_gae=False):
        ...
        self.policy = LRCC_ActorCritic(action_dim, exploration_param, self.device).to(self.device)
        self.optimizer = torch.optim.Adam(self.policy.parameters(), lr=lr, betas=betas)

        self.policy_old = LRCC_ActorCritic(action_dim, exploration_param, self.device).to(self.device)
