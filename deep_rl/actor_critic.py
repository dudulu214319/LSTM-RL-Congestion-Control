...
class ActorCritic(nn.Module):
    ...
    def forward(self, state): # 只有policy_old执行该函数，off_policy架构
    ...
    def evaluate(self, state, action):
        action_mean = self.actor(state)
        cov_mat = torch.diag(self.action_var).to(self.device)
        dist = MultivariateNormal(action_mean, cov_mat) # 重要性采样
...
class LRCC_ActorCritic(nn.Module):
    def __init__(self, action_dim, exploration_param=0.05, device="cpu"):
        super(LRCC_ActorCritic, self).__init__()
        # output of actor in [0, 1]
        self.layer1_shape = 128
        self.layer2_shape = 128
        self.numFcInput = 1664
        ...
        self.oConv_critic = nn.Linear(1, 128)
    ...
    def forward(self, inputs): # 只有policy_old执行该函数，off_policy架构
        # actor
        ...
        lstmoutputConv = F.relu(self.oConv(inputs[:, 4:5, -1].reshape(-1,1)), inplace=True)
        ...
        lstmoutput_flatten = lstmoutputConv.view(lstmoutputConv.shape[0], -1)

        merge = torch.cat([receiving_flatten, delay_flatten, loss_flatten, predication_flatten,lstmoutput_flatten], 1)
        ...
        # overusedisConv = F.relu(self.oConv1d(inputs[:, 4:5, :]), inplace=True)
        lstmoutputConv_critic = F.relu(self.oConv_critic(inputs[:, 4:5, -1].reshape(-1,1)), inplace=True)
        ...
        # overusedis_flatten = overusedisConv.view( overusedisConv.shape[0], -1)
        lstmoutput_flatten_critic = lstmoutputConv_critic.view(lstmoutputConv_critic.shape[0], -1)

        merge_critic = torch.cat([receiving_flatten_critic, delay_flatten_critic, loss_flatten_critic, predication_flatten_critic,lstmoutput_flatten_critic], 1)
